    ...
    with torch.no_grad():
        ...
        for batch_idx, (x, _) in enumerate(pbar):
            ...
            if batch_idx == len(val_loader) - 1:
                ...
                # Same size: upsample ground truth to match model output
                x_up = F.interpolate(x[:8], size=(model.output_size, model.output_size),
                                    mode='bilinear', align_corners=False)
                comparison = torch.cat([x_up, x_hat[:8]])
                save_image(comparison.cpu(), f"results/val_recon_epoch_{epoch+1}.png", nrow=8)

                # Double scaled: just nearest-neighbour scale up both for visibility
                x_display = F.interpolate(x_up, scale_factor=2, mode='nearest')
                x_hat_display = F.interpolate(x_hat[:8], scale_factor=2, mode='nearest')
                comparison_2x = torch.cat([x_display, x_hat_display])
                save_image(comparison_2x.cpu(), f"results/val_recon_epoch_{epoch+1}_2x.png", nrow=8)
...
def train(num_epochs, batch_size, learning_rate, embedding_dim, num_embeddings, 
    ...
    model = VQVAE(
        ...
        modes=4,
        output_size=56      # 28=same res, 56=2x super-res, 112=4x
    )
